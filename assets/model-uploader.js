...
(function() {
  ...
  async function submitToDraftOrderMultiFile() {
    ...
    let uploadResults = [];
    try {
      const API_BASE = window.QUOTES_API_BASE || 'shopify-test-git-issac-branch-sheepxixis-projects.vercel.app/api';
    ...
    // 第二步：创建草稿订单
    try {
      const API_BASE = 'shopify-test-git-issac-branch-sheepxixis-projects.vercel.app/api';
  ...
  async function addFileToCart(fileId, fileData) {
    ...
    try {
      const base = (window.QUOTES_API_BASE || 'shopify-test-git-issac-branch-sheepxixis-projects.vercel.app/api').replace(/\/$/, '');
